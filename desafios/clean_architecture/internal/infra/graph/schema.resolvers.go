...
import (
	...
	"github.com/rsilraf/pos_goexpert/desafios/clean_architecture/internal/infra/graph/model"
...
func (r *queryResolver) ListOrders(ctx context.Context) ([]*model.Order, error) {
	ordersUC, err := r.ListOrdersUseCase.Execute()
	if err != nil {
		log.Println("error on list orders UC call")
		return nil, err
	}
	orders := []*model.Order{}

	for _, order := range ordersUC {
